...
from streaming_modules import mocked
from streaming_modules.alpaca_batch import AlpacaNewsBatchInput
from streaming_modules.alpaca_stream import AlpacaNewsStreamInput
from streaming_modules.embeddings import EmbeddingModelSingleton
from streaming_modules.models import NewsArticle
from streaming_modules.qdrant import QdrantVectorOutput
...
def build(
    ...
    flow.input(
        ...
        ), # If is_batch==True, need from_ and to_datetime
    )

    # Apply transformations to input data
    flow.flat_map(lambda messages: parse_obj_as(List[NewsArticle], messages)) # Parse input messages into a list of NewsArticle objects
    if debug:
        flow.inspect(print)
    flow.map(lambda article: article.to_document()) # Convert each article into a document
    flow.map(lambda document: document.compute_chunks(model)) # Compute chunks for each document
    flow.map(lambda document: document.compute_embeddings(model)) # Compute embeddings for each document using pre-trained transformer model

    flow.output("output", _build_output(model, in_memory=debug)) # If in_memory==True, return a QdrantVectorOutput with in-memory storage
